...
int numero_seguidos(TGrafo *g, char *nome) {
    int seguidos = 0;
    TVizinho *aresta = busca_vertice(g, nome)->prim_vizinho;
    while(aresta != NULL){
        seguidos++;
        aresta = aresta->prox;
    }
...
int seguidores(TGrafo *vertice, char *nome, int imprime) {
    int seguidores = 0;
    while(vertice != NULL){
        TVizinho *aresta = vertice->prim_vizinho;
        while(aresta != NULL){
            if(strcmp(aresta->nome,nome) == 0){
                seguidores++;
                if(imprime == 1)
...
TGrafo *mais_popular(TGrafo *g) {
    TGrafo *popular = g;
    TGrafo *vertice = g->prox;
    while(vertice != NULL){
        if(seguidores(g, vertice->nome, 0) > seguidores(g, popular->nome, 0))
            popular = vertice;
        vertice = vertice->prox;
    }
    return popular;
...
int segue_mais_velho(TGrafo *g, int imprime) {
    ...
    while(vertice != NULL){
        ...
        if(aresta == NULL) 
            imprime = 0;
        while(aresta != NULL){
            if((busca_vertice(g, aresta->nome))->idade < vertice->idade)
                imprime = 0;
            aresta = aresta->prox;
        } 
